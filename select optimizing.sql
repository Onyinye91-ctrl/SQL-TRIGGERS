...
-- CREATE TABLE Orders
CREATE TABLE Orders(OrderID INT NOT NULL, ClientID VARCHAR(10) DEFAULT NULL, ProductID VARCHAR(10) DEFAULT NULL, Quantity INT DEFAULT NULL,
Cost DECIMAL(6,2) DEFAULT NULL, Date DATE DEFAULT NULL, PRIMARY KEY (OrderID)); 

-- CREATE TABLE EMPLOYEES
CREATE TABLE Employees(EmployeeID INT DEFAULT NULL, FullName VARCHAR(100) DEFAULT NULL, Role VARCHAR(50) DEFAULT NULL,
Department VARCHAR(255) DEFAULT NULL);

-- INSERT DATE INTO Orders TABLE
...
-- However, the data they need is contained within the OrderID, ProductID, Quantity and Date columns.
-- So, these are the important fields that their query must target. Targeting other columns in the table is an inefficient use of resources.
...
-- However, this query's execution plan shows that it does not use an index to perform this search, as indicated by the NULL values in
-- possible_keys and keys columns

EXPLAIN SELECT * FROM Orders WHERE ClientID = 'Cl1';

-- Help Lucky Shrub to optimize this query by creating an index named IdxClientID on the required column of the Orders table.
-- Once you have created the index, run the same SELECT statement as above with the EXPLAIN statement.

CREATE INDEX IdxClientID ON Orders(ClientID);
 -- Test the efficeincy of the index created
EXPLAIN SELECT * FROM Orders WHERE ClientID = 'Cl1';
...
-- However, there’s an index on the FullName column which the query cannot use because it contains a leading wildcard (%) in the WHERE
-- clause condition. The following steps can be taken to eliminate the leading wildcard:
...
-- create a new column ReverseFullName
ALTER TABLE Employees ADD COLUMN ReverseFullName VARCHAR(200);

-- check to see the column is created
show columns from Employees;

-- add values to the new column created
...
-- Run the select where clause again
SELECT * FROM Employees WHERE FullName LIKE '%Tolo';
